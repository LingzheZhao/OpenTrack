...
class SingleData:
    ...
    qvel: Union[jax.Array, np.ndarray] = struct.field(default_factory=lambda: jnp.empty(0))
...
class TrajectoryData(SingleData):
    ...
    def get_fast(self, traj_index, sub_traj_index, backend: ModuleType = jnp):
        ...
        return backend.squeeze(self.qpos[ind].copy()), backend.squeeze(self.qvel[ind].copy())
    ...
    def to_jax_fp8(self):
        ...
        for key, value in dic.items():
            if key != "split_points":
                dic[key] = jnp.array(value, dtype=jnp.float8_e4m3fnuz)
            else:
                dic[key] = jnp.array(value)
        return TrajectoryData(**dic)
...
def calculate_root_angular_velocity(qpos: jax.Array, frequency: float, backend: ModuleType = jnp):
    """
    ...
    return freejoint_angvel

def calculate_root_linear_velocity(qpos: jax.Array, frequency: float, backend: ModuleType = jnp):
    """
...
def calculate_joint_velocity(qpos: jax.Array, frequency: float, backend: ModuleType = jnp):
    """

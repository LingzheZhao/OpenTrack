...
tmj.registry.register("G1TrackingGeneral", "tracking_adapter_config")(g1_tracking_general_task_config())
...
@tmj.registry.register("G1TrackingGeneral", "tracking_adapter_train_env_class")
class G1TrackingGeneralEnv(g1_base.G1Env):
    ...
    def _get_obs(self, data: mjx.Data, traj_data: TrajectoryData, info: dict[str, Any]) -> mjx_env.Observation:
        ...
        avg_joint_vel = joint_vel[self.obs_joint_ids]
        ...
        privileged_state_dict = {
            ...
            "ref_gyro_pelvis": traj_data.cvel[self.body_id_pelvis, :3] * self._config.obs_scales_config.joint_vel,
            "ref_gvec_pelvis": traj_data.site_xmat[self._pelvis_imu_site_id].reshape(3, 3).T @ jp.array([0, 0, -1]),
            "ref_root_height": traj_data.qpos[2],
            "ref_joint_pos": traj_data.qpos[7:][self.obs_joint_ids] - self._default_qpos[self.obs_joint_ids],
            "ref_joint_vel": traj_data.qvel[6:][self.obs_joint_ids] * self._config.obs_scales_config.joint_vel,
        }
        ...
        world_state = jp.hstack([privileged_state_dict[k] for k in self._config.world_model_obs_keys])
        reference_world_state = jp.hstack([privileged_state_dict[k] for k in self._config.reference_world_model_obs_key])

        # Nan to 0
        state = jp.nan_to_num(state)
        privileged_state = jp.nan_to_num(privileged_state)
        current_history = jp.nan_to_num(current_history)
        world_state = jp.nan_to_num(world_state)
        reference_world_state = jp.nan_to_num(reference_world_state)

        return {
            "state": state,
            "privileged_state": privileged_state,
            "world_state": world_state,
            "ref_world_state": reference_world_state,
        }, current_history
